...
def simple_single_flow_experiment_Cubic(config):
    ...
    estimated_max_time = (config["num_packets"] * 1500 * 8) / (bandwidth_mbps * 1e6) + prop_delay * 2 + config["num_packets"] * 2
    # Run the simulation for a specified number of packets
    sim_env.run(until=estimated_max_time)  # Run for the estimated total time

    # After simulation, we can analyze logs or print summary statistics
    logger.write_all_logs(config["folder"])
